...
// so keep these labels in sync with ALL_RATING_HEADERS in Code.gs.
const RATING_SETS = {
  "PTE Mock Test": [
...
let isSubmitting = false; // guards against duplicate submissions
...
// Uses a real (readable) POST request rather than no-cors, so the app can
// actually confirm success or failure instead of guessing. Sending the
// JSON body with Content-Type "text/plain" keeps the request a "simple"
// cross-origin request, which avoids the CORS preflight (OPTIONS) call
// that Apps Script Web Apps cannot answer — that preflight failure was
// the root cause of the "Network Error" / POST incompatibility issues.
form.addEventListener("submit", async (e) => {
  e.preventDefault();

  if (isSubmitting) return; // guard against double-clicks / duplicate submits
  ...
  try {
    ...
    if (!result.success) {
      throw new Error(result.error || "The server rejected this submission.");
    }
    ...
    showErrorToast(
      err.message && err.message.includes("status")
        ...
        : (err.message || "Something went wrong while sending your feedback. Please check your connection and try again.")
    );
    setSendingState(false);
  }
});
